...
src/helpers/__tests__/       # Vitest tests for helpers (exercise-resolver, program-helpers, stats-calculator)
...
programs (user_id FK nullable, name, is_active) → program_versions → program_days → program_day_exercises (rest_seconds, group_type)
  → user_id NULL = global template program; user_id set = user-owned program
sessions (user_id FK, started_at, ended_at, tags TEXT[], deleted_at) → session_exercises (rest_seconds) → sets (notes)
...
15 migrations in `src/db/migrations/` (001–015). Each file is self-describing — read the SQL for details. Auto-applied on server startup via `runMigrations()`.
